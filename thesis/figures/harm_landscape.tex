...
\node[font=\normalsize\bfseries, text=white]
  at (\hw/2+0.15, -0.5) {COGNITIVE HARMS (13)};
...
\node[font=\normalsize\bfseries, text=white]
  at (\rx+\hw/2+0.15, -0.5) {AFFECTIVE HARMS (8)};
...
\node[font=\normalsize\bfseries, text=black!80]
  at (\hw/2+0.15, \by-0.5) {EQUITY HARMS (6)};
...
\node[font=\normalsize\bfseries, text=white]
  at (\rx+\hw/2+0.15, \by-0.5) {EPISTEMIC HARMS (5)};
...
\caption{Harm typology of gamification in history education. Negative effects from the 74-study corpus are systematised into four categories: cognitive, affective, equity, and epistemic harms. Numbers indicate studies reporting each subcategory; 60 of 74 studies (81.1\%) reported at least one negative effect.}
\label{fig:harm-typology}
\end{figure}
