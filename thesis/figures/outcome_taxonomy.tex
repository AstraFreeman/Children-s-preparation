...
\draw[-stealth, thick, black!50] (\rightcol + 2.0, \yA + 0.15) -- (\rightcol + 2.0, \yE - 0.15);
\node[rotate=-90, anchor=south, font=\scriptsize, text=black!50] at (\rightcol + 2.2, {(\yA + \yE)/2})
    {Difficulty};

% --- Level 1: Engagement ---
\node[level, fill=blue!22, minimum width=\wA cm] (L1) at (0, \yA)
    {\textbf{Engagement}};
...
\node[level, fill=blue!12, minimum width=\wB cm] (L2) at (0, \yB)
...
\node[level, fill=orange!12, minimum width=\wC cm] (L3) at (0, \yC)
...
% --- Level 4: Motivation ---
\node[level, fill=orange!18, minimum width=\wD cm] (L4) at (0, \yD)
    {\textbf{Motivation}};
...
\node[level, fill=red!15, minimum width=\wE cm] (L5) at (0, \yE)
